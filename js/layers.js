...
addLayer("c", {
    ...
    autoPrestige() {return player.c.autoPrestige},
...
addLayer("f", {
    ...
    branches: ['c'],
    hotkeys: [
        {key: "f", description: "F: Reset for functions", onPress(){if (canReset(this.layer)) doReset(this.layer)}, unlocked() {return player[this.layer].unlocked}},
    ],
    layerShown(){return hasUpgrade('c', 33) || player[this.layer].unlocked},
    autoPrestige() {
    ...
    tabFormat: {
        ...
        Loops: {
            content: [
                ...
                "resource-display",
                "blank",
                ["display-text", () => {return "You have <h2 style='color: #00FFCC'>"+formatWhole(player.f.loops)+"</h2> Loops<br>Your loops increase the effective functions for effects by +"+format(player.f.loops)+" and unlock a buyable at each loop up to [x]"}],
                "blank",
                ["bar", "barA"],
                "blank",
                "buyables",
    ...
    milestones: {
        0: {
            ...
            effectDescription() {return "Multiply point gain by e^f<br>Currently: x"+format(milestoneEffect(this.layer, this.id))},
        ...
        1: {
            ...
            effectDescription() {return "Multiply constant gain by f^2<br>Currently: x"+format(milestoneEffect(this.layer, this.id))},
        ...
        2: {
            ...
            effectDescription() {return "Multiply point gain by log10(c)<br>Currently: x"+format(milestoneEffect(this.layer, this.id))},
        ...
        3: {
            ...
            effectDescription() {return "Keep all constant upgrades on reset and automatically buy 'Natural' without costing anything"},
            done() {return player.f.loops.gte(2)},
            unlocked() {return player.f.loops.gte(1) || hasMilestone(this.layer, this.id)},
        },
        4: {
            requirementDescription: "Loop 4: Self Sustaining",
            effectDescription() {return "Automatically reset for functions and they reset nothing"},
            done() {return player.f.loops.gte(4)},
            unlocked() {return player.f.loops.gte(1) || hasMilestone(this.layer, this.id)},
        },
        5: {
            requirementDescription: "Loop 5: Unlocking",
            effectDescription() {return "Unlock a new layer"},
            done() {return player.f.loops.gte(5)},
            unlocked() {return player.f.loops.gte(1) || hasMilestone(this.layer, this.id)},
        },
    },
    bars: {
        barA: {
            ...
            height: 50,
            progress() {
                return player.f.points.sub(player.f.loops.pow(1.3).mul(5).ceil()).div(player.f.loops.add(1).pow(1.3).mul(5).ceil().sub(player.f.loops.pow(1.3).mul(5).ceil()))
            },
            display() {
                return formatWhole(player.f.points)+" / "+formatWhole(player.f.loops.add(1).pow(1.3).mul(5).ceil())+" Functions"
            },
            fillStyle() {return{
    ...
    buyables: {
        11: {
            title() {return formatWhole(getBuyableAmount(this.layer, this.id))+"<br>Loop 1"},
            display() {
                return `
            ...
            effect() {
                return getBuyableAmount(this.layer, this.id).pow_base(player.f.loops.max(0).root(2).add(2))
            },
            unlocked() {return player.f.loops.gte(1) || getBuyableAmount(this.layer, this.id).gte(1)},
            tooltip: "Effect Formula:<br>(sqrt (loops) + 2) ^ x<br><br>Cost Formula:<br>25 ^ x ^ 131 * 1e15",
        },
        12: {
            title() {return formatWhole(getBuyableAmount(this.layer, this.id))+"<br>Loop 2"},
            display() {
                return `
            ...
            effect() {
                return getBuyableAmount(this.layer, this.id).pow_base(player.points.max(0).add(1).log(10).add(1))
            },
            unlocked() {return player.f.loops.gte(2) || getBuyableAmount(this.layer, this.id).gte(1)},
            tooltip: "Effect Formula:<br>(log10 (points) + 1) ^ x<br><br>Cost Formula:<br>25 ^ x ^ 1.3 * 1e25",
        },
        13: {
            title() {return formatWhole(getBuyableAmount(this.layer, this.id))+"<br>Loop 3"},
            display() {
                return `
                Increase effective levels of 'Natural' based on loops
                Cost: ${format(this.cost())} constants
                Effect: +${format(buyableEffect(this.layer, this.id))} 'Natural' levels
                `
            },
            cost(x = getBuyableAmount(this.layer, this.id)) {
            ...
            effect() {
                return getBuyableAmount(this.layer, this.id).root(1.08).mul(player.f.loops.max(0).add(1).root(2))
            },
            unlocked() {return player.f.loops.gte(3) || getBuyableAmount(this.layer, this.id).gte(1)},
            tooltip: "Effect Formula:<br>1.08rt(x)(sqrt (loops))<br><br>Cost Formula:<br>25 ^ x ^ 1.3 * 1e35",
        },
    },
})
